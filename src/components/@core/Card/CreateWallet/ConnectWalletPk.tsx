...
export default function ConnectWalletPk({ closeAllModals , onCreate}: ConnectWalletProps) {
    ...
    const connectWallet = async () => {
        if (!password || !privateKey) {
            setErr("Password and Private Key are required!");
            return;
        }

        try {
            const wallet = await connectWalletWithPk(privateKey, password);
    ...
    return (
        <div className="relative bg-[#11314a] rounded-xl shadow-lg p-6 flex flex-col items-center justify-center max-w-xl">
            ...
            <label className="mt-2">Private Key</label>
            <input
                type="password"
                value={privateKey}
